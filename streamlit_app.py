...
def image_analysis():
                    ...
                    try:
                        ...
                        if pose_frame.success:
                            ...
                            with col2:
                                ...
                                if analysis and isinstance(analysis, dict) and "exercise" in analysis:
                                    display_frame_analysis(analysis, show_features=True)
                                else:
                                    st.warning("⚠️ Could not analyze pose features properly.")
                        else:
    ...
    with col2:
        if st.session_state.analysis_results:
            ...
            if isinstance(analysis, dict) and "exercise" in analysis and "confidence" in analysis:
                st.markdown("**Result Summary**")
                st.metric("Exercise", analysis["exercise"])
                st.metric("Confidence", f"{analysis['confidence']:.2%}")
            else:
                st.warning("⚠️ Analysis results are incomplete or invalid.")
